...
unsigned char trenutnoStanjeSoftP01_zaMain = 1;					  // ukljuci / iskljuci
...
unsigned char trenutnoStanjeSoftP02_zaMain = 1;					  // prg 
...
unsigned char trenutnoStanjeSoftP03_zaMain = 1;					   // brz
...
{
	if(prijem[0] == '(' && prijem[1] == '1' && prijem[2] == ':' && prijem[11] == ')')
	{
	...
	else if(prijem[0] == '(' && prijem[1] == '2' && prijem[2] == ':' && prijem[11] == ')')
	{
...
void main(void)
{
	...
	unsigned char niz_stanja4[7] = {0x03, 0x06, 0x0C, 0x30, 0x60, 0xC0, 0x81}; // 7 == 4
	...
	EA = 1;
	TR0 = 1;
	// t2 mod T2CON = 34h | RCAP2H, RCAP2L / 0FFh 0FDh
...
(1:por) - komanda za start ("prvi red": "poruka za ispis u prvom redu")
(2:por) - isto samo u drugom redu

transmit ove poruke, za proveru da li je poslata(ta ista ili neka druga)
kad je primljena ispravno

onda write line sa izdvojenom porukom

protokol je bitan za parsiranje
*/