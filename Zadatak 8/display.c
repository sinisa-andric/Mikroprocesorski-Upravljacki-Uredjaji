...
void wait2ms(void)
{
	...
	TH0 = 0x0F8; //isto kao prethodno samo brojis 2ms
	TL0 = 0xCD;
	TR0 = 1;
	while (!TF0)
	{
...
void writeLine(unsigned char *message)
{ //funkcija koja pise jednu rec na displej- iterira kroz string i poziva funkciju writeChar za svako slovo
	unsigned char n = 0;
	while (message[n] != '\0')
	{
		writeChar(message[n]);
